...
const Manager = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      await fetchDepartments(); // Wait for departments to be fetched
      await getData(); // Then fetch manager data
  ...
  const getData = async () => {
    try {
      const result = await axios.get("https://lms-be-beta.vercel.app/api/Manager");
      setData(result.data);
    } catch (error) {
