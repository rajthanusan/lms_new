...
const Employees = () => {
  ...
  // Fetch user data based on department
  ...
  // Get department based on logged-in manager's username
  useEffect(() => {
  ...
  // Fetch data when the department changes
  useEffect(() => {
    if (department) {
      getData();
    } else {
      setLoading(false); // Stop loading if no department is available
    }
  }, [department, getData]);
    ...
    <Fragment>
      ...
      <Container className="my-5">
        ...
        {loading ? ( // Show loading while fetching data
          ...
          hasFetched && ( // Only show this after data has been fetched
            currentRows.length > 0 ? (
              ...
              <div className="text-center">No employee records found.</div>
            )
          )
        )}
          ...
          <Modal.Body>
            {selectedEmployee && (
              <div>
                ...
                <p>Birthdate: {new Date(selectedEmployee.birthday).toLocaleDateString()}</p>
                <p>Joined date: {new Date(selectedEmployee.joindate).toLocaleDateString()}</p>
        ...
        <Modal show={showDeleteModal} onHide={handleCloseDeleteModal}>
          <Modal.Header closeButton>
            <Modal.Title className="text-darkblue">Delete Employee</Modal.Title>
          </Modal.Header>
          <Modal.Body>
